...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`pages_rels_news_id_idx\` ON \`pages_rels\` (\`news_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_pages_v_rels_news_id_idx\` ON \`_pages_v_rels\` (\`news_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`peoples_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`url\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`peoples\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`peoples_links_order_idx\` ON \`peoples_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`peoples_links_parent_id_idx\` ON \`peoples_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`peoples\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`image_id\` integer,
  	\`external_image_url\` text,
  	\`name\` text NOT NULL,
  	\`instation\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`peoples_image_idx\` ON \`peoples\` (\`image_id\`);`)
  await db.run(sql`CREATE INDEX \`peoples_updated_at_idx\` ON \`peoples\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`peoples_created_at_idx\` ON \`peoples\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`peoples_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  	\`roles_id\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`peoples\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`roles_id\`) REFERENCES \`roles\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`peoples_rels_order_idx\` ON \`peoples_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`peoples_rels_parent_idx\` ON \`peoples_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`peoples_rels_path_idx\` ON \`peoples_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`peoples_rels_roles_id_idx\` ON \`peoples_rels\` (\`roles_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`search_rels_news_id_idx\` ON \`search_rels\` (\`news_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`roles_id\`) REFERENCES \`roles\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`peoples_id\`) REFERENCES \`peoples\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_peoples_id_idx\` ON \`payload_locked_documents_rels\` (\`peoples_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`footer_link_groups_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`link_type\` text DEFAULT 'reference',
  	...
  	\`link_url\` text,
  	\`link_label\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`footer_link_groups\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`footer_link_groups_links_order_idx\` ON \`footer_link_groups_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`footer_link_groups_links_parent_id_idx\` ON \`footer_link_groups_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`footer_link_groups\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_link_groups_order_idx\` ON \`footer_link_groups\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`footer_link_groups_parent_id_idx\` ON \`footer_link_groups\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`footer\` (
  	...
  	\`brand_name\` text DEFAULT 'ARCES',
  	\`description\` text,
  	\`newsletter_title\` text DEFAULT 'Newsletter',
  	\`newsletter_description\` text,
  	\`copyright_text\` text DEFAULT '©{year} Arces. All rights reserved',
  	\`updated_at\` text,
  	\`created_at\` text
  );
  `)
  ...
  await db.run(sql`CREATE TABLE \`tentang_visi_misi_misions\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`text\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`tentang\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`tentang_visi_misi_misions_order_idx\` ON \`tentang_visi_misi_misions\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`tentang_visi_misi_misions_parent_id_idx\` ON \`tentang_visi_misi_misions\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tentang_peran_strategis_cards\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`tentang_peran_strategis_cards_order_idx\` ON \`tentang_peran_strategis_cards\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`tentang_peran_strategis_cards_parent_id_idx\` ON \`tentang_peran_strategis_cards\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tentang\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`sejarah_image_id\` integer,
  	\`sejarah_content\` text NOT NULL,
  	\`visi_misi_visi\` text NOT NULL,
  	\`peran_strategis_image_id\` integer,
  	\`website_content\` text NOT NULL,
  	\`physical_address_content\` text NOT NULL,
  	\`physical_address_maps_embed_url\` text,
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`sejarah_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`peran_strategis_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`tentang_sejarah_sejarah_image_idx\` ON \`tentang\` (\`sejarah_image_id\`);`)
  await db.run(sql`CREATE INDEX \`tentang_peran_strategis_peran_strategis_image_idx\` ON \`tentang\` (\`peran_strategis_image_id\`);`)
  await db.run(sql`CREATE TABLE \`kontak_layanan_layanan_services\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	...
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`kontak_layanan\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`kontak_layanan_layanan_services_order_idx\` ON \`kontak_layanan_layanan_services\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`kontak_layanan_layanan_services_parent_id_idx\` ON \`kontak_layanan_layanan_services\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`kontak_layanan\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`header_title\` text DEFAULT 'Kontak & Layanan' NOT NULL,
  	\`header_description\` text NOT NULL,
  	\`layanan_title\` text DEFAULT 'Our Services' NOT NULL,
  	\`kontak_title\` text DEFAULT 'Contact Information' NOT NULL,
  	\`kontak_email\` text NOT NULL,
  	\`kontak_email_label\` text DEFAULT 'Email Editorial',
  	\`kontak_address\` text NOT NULL,
  	\`kontak_address_label\` text DEFAULT 'Office Location',
  	\`kontak_phone\` text NOT NULL,
  	\`kontak_phone_label\` text DEFAULT 'WhatsApp Support',
  	\`kontak_maps_embed_url\` text,
  	\`kontak_right_content\` text,
  	\`updated_at\` text,
  	\`created_at\` text
  );
  `)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`peoples_links\`;`)
  await db.run(sql`DROP TABLE \`peoples\`;`)
  await db.run(sql`DROP TABLE \`peoples_rels\`;`)
  ...
  await db.run(sql`DROP TABLE \`footer_link_groups_links\`;`)
  await db.run(sql`DROP TABLE \`footer_link_groups\`;`)
  await db.run(sql`DROP TABLE \`footer_bottom_links\`;`)
  await db.run(sql`DROP TABLE \`footer\`;`)
  await db.run(sql`DROP TABLE \`footer_rels\`;`)
  await db.run(sql`DROP TABLE \`tentang_visi_misi_misions\`;`)
  await db.run(sql`DROP TABLE \`tentang_peran_strategis_cards\`;`)
  await db.run(sql`DROP TABLE \`tentang\`;`)
  await db.run(sql`DROP TABLE \`kontak_layanan_layanan_services\`;`)
  await db.run(sql`DROP TABLE \`kontak_layanan\`;`)
}
