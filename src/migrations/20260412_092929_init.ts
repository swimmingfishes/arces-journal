...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`news_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`news_populated_authors_order_idx\` ON \`news_populated_authors\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`news_populated_authors_parent_id_idx\` ON \`news_populated_authors\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_news_v_version_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_uuid\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_news_v_version_populated_authors_order_idx\` ON \`_news_v_version_populated_authors\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_news_v_version_populated_authors_parent_id_idx\` ON \`_news_v_version_populated_authors\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`peoples_role_idx\` ON \`peoples\` (\`role_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`news_populated_authors\`;`)
  await db.run(sql`DROP TABLE \`news\`;`)
  await db.run(sql`DROP TABLE \`news_rels\`;`)
  await db.run(sql`DROP TABLE \`_news_v_version_populated_authors\`;`)
